...
def paramètre(arg1, arg2, arg3, i):
    global const, arg4
    arg4[i] = DoubleVar()
    text = Label(arg1, text=arg2, width=10)
    text.grid(row=arg3, column=0)
    entree=Entry(arg1, textvariable=arg4[i], width=30)
    print(arg4[i])
    bouton=Button(arg1, text="Entrée", command=liste(i))
...
def données():
    ...
    paramètre(fen_1, "Densité (noté {0:^1s})".format(chr(961)), 6, 0)
    paramètre(fen_1, "Vitesse moyenne de la fibre", 7, 1)
    paramètre(fen_1, "Viscosité dynamique (noté {0:^1s})".format(chr(956)), 8, 2)
    paramètre(fen_1, "Rayon du papier", 9, 3)
